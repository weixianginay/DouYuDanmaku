...
1. 继续使用 [斗鱼 Socket 信息类型 1](message type 1.txt) 中的 Socket ，弹幕信息会在该 Socket 中返回。
2. 需新开通一个 Socket 连接弹幕服务器。
    需要以下基础数据：

    - 弹幕服务器地址与端口号
    - room id
    - gid
    - username （可以写成固定的内容，匿名用户的密码一直都是固定的 `1234567890123456`）

    以上数据，均可以从 [斗鱼 Socket 信息类型 1](message type 1.txt) 中获取。

方式 1 与方式 2 ，具体使用哪一种方式进行，暂时无法获知。但是，根据观察，我自己的直播间（观众人数为 1~5），走的是方式 1 。而 yyf 等名气主播（观众人数为数万级别），走的是方式 2 。

如果针对我自己的直播间走方式 2 ，会发现无法获取到弹幕，需要进入海量弹幕模式时（gid 设置为 -9999），才能获取到弹幕。

由于方式 1 中的心跳机制，包含了验证机制。验证机制暂时无法获知。所以，针对使用方式 1 进行的直播间，我们也将使用方式 2 进行，但是会进行海量弹幕模式。
